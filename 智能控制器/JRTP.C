...
interrupt [0x0b] void T0_int (void)
{
    ...
    if((ms++)==(1000/TIMEBASE-1)){
        ...
        if(solar_state<7){
            ...
            if(value<(solar_state==13?eprom(BOOST_BELOW):eprom(FLOAT_MIN))){
                if(next_state==solar_state+1){
                ...
                if(value>eprom(FLOAT_MAX) && solar_state!=7){
                    if(next_state==solar_state-1){
                        if(Delay_change==0)solar_state-=1;
                    }
                    else{
...
ToPrinter()
{
  ...
  aputchar(64);
    ...
    aprintf("\t充电状态 = %d \n",solar_state);
    for(i=0;i<6;i++)aprintf("太阳能电池第%d组电压 =  V\t电流 = %o A\n",i+1,ad_get(SR_I0+i));
    ...
    aprintf("\n强充最大电压 = %oV\t",eprom(6));
    aprintf("强充递减电压 = %oV\n",eprom(1));
    aprintf("浮充电压上限 = %oV\t",eprom(FLOAT_MAX));
    aprintf("浮充电压下限 = %oV\n",eprom(FLOAT_MIN));
